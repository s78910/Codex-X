((cssText, artDataUrl, themeConfig) => {
  ...
  const applyTheme = (root, shell) => {
    ...
    const surfaceOpacity = Number.isFinite(Number(THEME.surfaceOpacity))
      ? clamp(Number(THEME.surfaceOpacity), 0.35, 1)
      : 1;
  ...
  const applyArtMetadata = (root) => {
    ...
    const explicitFocusY = typeof ART.focusY === "number";
    ...
    const homeFocusY = !explicitFocusY && !wide && ["wide", "landscape"].includes(aspect)
      ? Math.min(focusY, 0.28) : focusY;
    const focusXValue = `${(clamp(focusX, 0, 1) * 100).toFixed(2)}%`;
    const focusYValue = `${(clamp(focusY, 0, 1) * 100).toFixed(2)}%`;
    const homeFocusYValue = `${(clamp(homeFocusY, 0, 1) * 100).toFixed(2)}%`;
    ...
    setStyleProperty(root, "--dream-skin-home-art-position", `${focusXValue} ${homeFocusYValue}`);
  };
