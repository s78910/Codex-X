...
assert.match(
  css,
  /\.dream-skin-home > div:has\(\[data-feature="game-source"\]\) > div:first-child > div:first-child\s*\{[\s\S]{0,700}background-position:\s*var\(--ds-home-art-position\) !important;/,
  "The home hero must use its route-specific focal point.",
);
assert.match(
  css,
  /\.dream-skin-home > div:has\(\[data-feature="game-source"\]\)\s*\{[\s\S]{0,120}min-height:\s*100% !important;/,
  "The home layout must target the content container instead of an inserted banner sibling.",
);
assert.doesNotMatch(
  css,
  /\.dream-skin-home > div:first-child\s*\{[\s\S]{0,120}min-height:\s*100% !important;/,
  "An empty home banner container must not push the composer below the viewport.",
);
assert.doesNotMatch(
  css,
...
assert.match(
  css,
  /--ds-immersive-composer-solid:\s*rgb\(var\(--ds-panel-rgb\) \/ \.94\);/,
  "The light composer should remain opaque enough to prevent thread text from showing through.",
);
assert.match(
  css,
  /--ds-task-immersive-edge:\s*rgb\(var\(--ds-panel-rgb\) \/ calc\(\.86 \* var\(--ds-surface-opacity\)\)\);/,
  "Wide task surfaces should expose the wallpaper according to the configured opacity.",
);
assert.match(
  css,
  /--ds-task-fade:\s*linear-gradient\(180deg,[\s\S]{0,400}var\(--ds-surface-opacity\)/,
  "Regular landscape artwork should apply the configured opacity to its task scrim.",
);
assert.match(
  css,
  /main\.main-surface:not\(\.dream-skin-home-shell\)::before\s*\{[\s\S]{0,420}opacity:\s*1;/,
  "The wallpaper layer must keep its original colors instead of blending through a white base.",
);
const homeSuggestionRuleStart = css.indexOf(
  ".dream-skin-home .group\\/home-suggestions button {",
);
assert.notEqual(homeSuggestionRuleStart, -1, "The home suggestion card rule must exist.");
const homeSuggestionRuleEnd = css.indexOf("}", homeSuggestionRuleStart);
const homeSuggestionRule = css.slice(homeSuggestionRuleStart, homeSuggestionRuleEnd);
assert.match(
  homeSuggestionRule,
  /background:\s*rgb\(var\(--ds-panel-rgb\) \/ calc\(\.56 \* var\(--ds-surface-opacity\)\)\) !important;/,
  "Home suggestion cards must use the shared surface opacity instead of a fixed white layer.",
);
assert.match(
  css,
  /--ds-composer-surface:\s*rgb\(var\(--ds-panel-rgb\) \/ calc\(\.82 \+ \(\.12 \* var\(--ds-surface-opacity\)\)\)\);/,
  "The composer must keep a readable minimum opacity while responding to the skin setting.",
);
assert.match(
  css,
  /html\.codex-dream-skin \.composer-surface-chrome\s*\{[\s\S]{0,100}overflow:\s*hidden !important;/,
  "Composer internals must be clipped to the rounded input surface.",
);
assert.match(
  css,
...
assert.match(
  template,
  /\[data-feature="game-source"\][\s\S]{0,180}\[data-codex-composer-root\]/,
  "Home detection should use stable semantic markers instead of optional suggestion cards.",
);
assert.match(
  css,
...
assert.equal(defaults.rootStyle.values.get("--ds-surface-opacity"), "1.00");
...
const autoLandscape = createFixture({
  id: "auto-landscape-focus",
  appearance: "auto",
  art: { safeArea: "auto", taskMode: "auto" },
  artMetadata: {
    width: 1586,
...
vm.runInNewContext(autoLandscape.payload, autoLandscape.context);
assert.equal(autoLandscape.rootStyle.values.get("--dream-art-position"), "50.00% 50.00%");
assert.equal(
  autoLandscape.rootStyle.values.get("--dream-skin-home-art-position"),
  "50.00% 28.00%",
  "An unconfigured landscape should keep upper subjects visible in the much wider home hero.",
);
...
assert.equal(explicit.rootStyle.values.get("--dream-skin-home-art-position"), "15.00% 80.00%");
assert.equal(explicit.rootStyle.values.get("--ds-surface-opacity"), "0.55");
